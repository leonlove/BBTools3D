...
set log_file=%dir_log%\build_proj.log
...
set package_name=proj-7.2.1.tar.gz
set package_dir=%dir_build%\proj
...
:: 创建cmake源码编译路径
set package_dir_build=%package_dir%\proj-7.2.1\build
if not exist %package_dir_build% (
	mkdir %package_dir_build%
)
cd %package_dir_build%

"%CMakeExePath%" .. -G %cmake_generator% -DCMAKE_INSTALL_PREFIX=%dir_install%\proj -DTIFF_INCLUDE_DIR=%dir_install%\tiff\include -DTIFF_LIBRARY=%dir_install%\tiff\lib\tiff.lib -DCURL_LIBRARY=%dir_install%\curl\lib\libcurl_imp.lib -DCURL_INCLUDE_DIR=%dir_install%\curl\include -DSQLITE3_LIBRARY=%dir_install%\sqlite\lib\sqlite3.lib -DSQLITE3_INCLUDE_DIR=%dir_install%\sqlite\include -DEXE_SQLITE3=%dir_install%\sqlite\tools\sqlite3-bin.exe -DBUILD_TESTING=off
