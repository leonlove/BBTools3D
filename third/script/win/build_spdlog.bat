...
set log_file=%dir_log%\build_spdlog.log
...
set package_name=spdlog-1.4.2.tar.gz
set package_dir=%dir_build%\spdlog
...
:: 创建cmake源码编译路径
set package_dir_build=%package_dir%\spdlog-1.4.2\build
if not exist %package_dir_build% (
	mkdir %package_dir_build%
)
cd %package_dir_build%

"%CMakeExePath%" .. -G %cmake_generator% -DCMAKE_INSTALL_PREFIX=%dir_install%\spdlog
