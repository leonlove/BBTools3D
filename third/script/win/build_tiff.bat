...
set log_file=%dir_log%\build_tiff.log
...
set package_name=tiff-4.1.0.tar.gz
set package_dir=%dir_build%\tiff
...
:: 创建cmake源码编译路径
set package_dir_build=%package_dir%\tiff-4.1.0\build
if not exist %package_dir_build% (
	mkdir %package_dir_build%
)
cd %package_dir_build%

"%CMakeExePath%" .. -G %cmake_generator% -DCMAKE_INSTALL_PREFIX=%dir_install%\tiff
