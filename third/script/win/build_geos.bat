...
set log_file=%dir_log%\build_geos.log
...
set package_name=geos-3.9.1.tar.bz2
set package_dir=%dir_build%\geos
...
:: 创建cmake源码编译路径
set package_dir_build=%package_dir%\geos-3.9.1\build
if not exist %package_dir_build% (
	mkdir %package_dir_build%
)
cd %package_dir_build%

"%CMakeExePath%" .. -G %cmake_generator% -DCMAKE_INSTALL_PREFIX=%dir_install%\geos
